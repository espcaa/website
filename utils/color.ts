...
export async function getMaterialWallpaperColorsMulti(
  ...
  let primarySeed: number;
  if (seedOverride) {
    // parse #rrggbb into argb
    const hex = seedOverride.replace("#", "");
    const rgb = parseInt(hex, 16);
    primarySeed = (0xff << 24) | rgb; // force alpha to 255
  } else {
    primarySeed = seedColors[0];
  }

  const secondarySeed = seedColors[1] ?? primarySeed;
  const tertiarySeed = seedColors[2] ?? primarySeed;
